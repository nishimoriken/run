...
### ASP encoding
my %encoding = ();
$encoding{"cfb"}  = "./target/release/cfb";
...
sub solve {
    my ($p, $enc, $slv) = @_;
    my ($problem, $col) = @{$p};
    my $log = &make_file_name($log_dir, $problem, $enc, $slv, $bound) . ".log";
...
sub run {
    my ($col, $enc, $slv, $log) = @_;
    my $cmd2 = join(" ", ($solver{$slv}, $encoding{$enc}, $col, $bound));
    print "$cmd2\n";
    if ($opt_n) {
	return;
    }
